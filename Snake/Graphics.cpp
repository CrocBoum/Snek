...
void Graphics::DrawLine(int x0, int y0, int x1, int y1, Color c) {
	...
	const float mForX = float(yDiff) / float(xDiff);
	const int tForX = y0;

	for (int i = 0; i <= xDiff; i++) {
		PutPixel(i + x0, int(std::round(mForX * float(i) + float(tForX))), c);
	}

	const float mForY = float(xDiff) / float(yDiff);
	const int tForY = x0;

	for (int i = 0; i <= yDiff; i++) {
		PutPixel(int(std::round(mForY * float(i) + float(tForY))), i + y0, c);
	}
}
...
void Graphics::DrawRectByCorners(int x0, int y0, int x1, int y1, Color c) {
...
void Graphics::DrawIsoRightTriUR(int x, int y, int size, Color c) {
	for (int ly = y; ly < y + size; ly++) {
		const int curLine = ly - y;
		for (int lx = x + curLine - size + 1; lx <= x; lx++) {
			PutPixel(lx, ly, c);
		}
	}
}
...
void Graphics::DrawIsoRightTriBL(int x, int y, int size, Color c) {
	for (int ly = y - size + 1; ly <= y; ly++) {
		const int curLine = y - ly;
		for (int lx = x; lx < x + size - curLine; lx++) {
			PutPixel(lx, ly, c);
		}
	}
...
void Graphics::DrawDonut(int cx, int cy, int rad, int width, Color c) {

	const int radSq = rad * rad;
	const int widthSq = width * width;
	for (int y = cy - rad; y <= cy + rad; y++) {
		for (int x = cx - rad; x <= cx + rad; x++) {

			const int xDiff = cx - x;
			const int yDiff = cy - y;
			const int distSq = xDiff * xDiff + yDiff * yDiff;

			if (distSq <= radSq && distSq >= widthSq) {
				PutPixel(x, y, c);
			}
		}
	}
